...
void Obstacles::renderObstacles(SDL_Renderer* renderer,std::string level) {
	SDL_Surface* tmpSurface = IMG_Load("assets/images/obstacle2.png");
	SDL_Texture* tmpTexture = SDL_CreateTextureFromSurface(renderer, tmpSurface);
	if (level == "easy")obstacles_level = obstacles_easy;
	else if (level == "medium")obstacles_level = obstacles_medium;
