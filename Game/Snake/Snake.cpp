...
bool Snake::eatCherry(Cherry& cherry, SDL_Renderer* renderer) {
	if (pause == false) {
		...
		checkDirection[firstTail % 10000] = checkDirection[firstTail % 10000 - 1];
		//}
...
for (int i = 1; i < tail_size; i++) {
	...
	pos tail_pos = tail[(lastTail + i) % 10000];
	if (checkCorner[(lastTail + i) % 10000] > 0) {
		//check the snake go up right
		if (tail[(lastTail + i + 1) % 10000].x > tail[(lastTail + i) % 10000].x &&
			tail[(lastTail + i - 1) % 10000].y > tail[(lastTail + i) % 10000].y) {
		...
		else if (tail[(lastTail + i + 1) % 10000].x > tail[(lastTail + i) % 10000].x &&
			tail[(lastTail + i - 1) % 10000].y < tail[(lastTail + i) % 10000].y) {
		...
		else if (tail[(lastTail + i) % 10000].y > tail[(lastTail + i + 1) % 10000].y &&
			tail[(lastTail + i) % 10000].x > tail[(lastTail + i - 1) % 10000].x) {
		...
		else if (tail[(lastTail + i + 1) % 10000].y < tail[(lastTail + i) % 10000].y &&
			tail[(lastTail + i - 1) % 10000].x > tail[(lastTail + i) % 10000].x) {
		...
		//check go in
		//up left
		if (tail[(lastTail + i) % 10000].y == 2 && tail[(lastTail + i + 1) % 10000].x < tail[(lastTail + i) % 10000].x &&
			tail[(lastTail + i - 1) % 10000].y == 23) {
			renderTail(renderer, tmpSurface, tmpTexture, "assets/images/leftDown.png", tail_pos);
		}
		// up right
		else if (tail[(lastTail + i) % 10000].y == 2 && tail[(lastTail + i + 1) % 10000].x > tail[(lastTail + i) % 10000].x &&
			tail[(lastTail + i - 1) % 10000].y == 23) {
			renderTail(renderer, tmpSurface, tmpTexture, "assets/images/rightDown.png", tail_pos);
		}
		//down left
		else if (tail[(lastTail + i) % 10000].y == 23 && tail[(lastTail + i + 1) % 10000].x < tail[(lastTail + i) % 10000].x &&
			tail[(lastTail + i - 1) % 10000].y == 2) {
			renderTail(renderer, tmpSurface, tmpTexture, "assets/images/leftUp.png", tail_pos);
		}
		//down right
		else if (tail[(lastTail + i) % 10000].y == 23 && tail[(lastTail + i + 1) % 10000].x > tail[(lastTail + i) % 10000].x &&
		...
		//left up
		else if (tail[(lastTail + i) % 10000].x == 0 && tail[(lastTail + i + 1) % 10000].y < tail[(lastTail + i) % 10000].y &&
			tail[(lastTail + i - 1) % 10000].x == 23) {
			renderTail(renderer, tmpSurface, tmpTexture, "assets/images/right_LeftUp.png", tail_pos);
		}
		//left down
		else if (tail[(lastTail + i) % 10000].x == 0 && tail[(lastTail + i + 1) % 10000].y > tail[(lastTail + i) % 10000].y &&
		...
		//right up
		else if (tail[(lastTail + i) % 10000].x == 23 && tail[(lastTail + i + 1) % 10000].y < tail[(lastTail + i) % 10000].y &&
			tail[(lastTail + i - 1) % 10000].x == 0) {
			renderTail(renderer, tmpSurface, tmpTexture, "assets/images/left_RightUp.png", tail_pos);
		}
		//right down
		else if (tail[(lastTail + i) % 10000].x == 23 && tail[(lastTail + i + 1) % 10000].y > tail[(lastTail + i) % 10000].y &&
		...
		//check go out
		if (tail[(lastTail + i) % 10000].y != 22 && tail[(lastTail + i) % 10000].y != 3 &&
			tail[(lastTail + i) % 10000].x != 1 && tail[(lastTail + i) % 10000].x != 22) {
			//=======check the snake go up ==========
			//check the snake go left
			if (tail[(lastTail + i) % 10000].y < tail[(lastTail + i + 1) % 10000].y &&
				tail[(lastTail + i) % 10000].x < tail[(lastTail + i - 1) % 10000].x &&
				tail[(lastTail + i + 1) % 10000].y == 23 && tail[(lastTail + i) % 10000].y == 2) {
				renderTail(renderer, tmpSurface, tmpTexture, "assets/images/left_RightUp.png", tail_pos);
			}
			...
			//check the snake go left
				...
				tail[(lastTail + i) % 10000].x < tail[(lastTail + i - 1) % 10000].x &&
				tail[(lastTail + i) % 10000].y == 23 && tail[(lastTail + i + 1) % 10000].y == 2) {
				renderTail(renderer, tmpSurface, tmpTexture, "assets/images/left_LeftDown.png", tail_pos);
			}
			//check the snake go right
			else if (tail[(lastTail + i) % 10000].y > tail[(lastTail + i + 1) % 10000].y &&
			...
			//check the snake go up
				...
				tail[(lastTail + i) % 10000].x < tail[(lastTail + i + 1) % 10000].x &&
				tail[(lastTail + i + 1) % 10000].x == 23 && tail[(lastTail + i) % 10000].x == 0) {
				renderTail(renderer, tmpSurface, tmpTexture, "assets/images/leftUp.png", tail_pos);
			}
			//check the snake go down
			else if (tail[(lastTail + i) % 10000].y > tail[(lastTail + i - 1) % 10000].y &&
			...
			//check the snake go up
				...
				tail[(lastTail + i) % 10000].x > tail[(lastTail + i + 1) % 10000].x &&
				tail[(lastTail + i + 1) % 10000].x == 0 && tail[(lastTail + i) % 10000].x == 23) {
				renderTail(renderer, tmpSurface, tmpTexture, "assets/images/rightUp.png", tail_pos);
			}
			//check the snake go down
			else if (tail[(lastTail + i) % 10000].y > tail[(lastTail + i - 1) % 10000].y &&
	...
	//tail normal handle
		...
		if (checkDirection[(lastTail + i) % 10000] == 1)
			renderAngle(renderer, tmpTexture, desrect.x, desrect.y, desrect.w, desrect.h, 90);
		else if ((checkDirection[(lastTail + i) % 10000] == 2))
...
if (tail_size > 0)
	for (int i = 0; i < 1; i++) {
		...
		//checkDirection[(lastTail + i) % 10000] == 1 -> up or down
		//checkDirection[(lastTail + i) % 10000] == 2 -> left or right

		//check the snake go up
		if (tail[(lastTail + i) % 10000].y == 2 && checkDirection[(lastTail + i) % 10000] == 1 &&
			tail[(lastTail + i + 1) % 10000].y == 23) {
			renderAngle(renderer, tailTexture, desrect.x, desrect.y, desrect.w, desrect.h, -90);
		}
		//check the snake go down
		else if (tail[(lastTail + i) % 10000].y == 23 && checkDirection[(lastTail + i) % 10000] == 1 &&
			tail[(lastTail + i + 1) % 10000].y == 2) {
			renderAngle(renderer, tailTexture, desrect.x, desrect.y, desrect.w, desrect.h, 90);
		}
		//check the snake go left
		else if (tail[(lastTail + i) % 10000].x == 0 && checkDirection[(lastTail + i) % 10000] == 2 &&
			tail[(lastTail + i + 1) % 10000].x == 23) {
			renderAngle(renderer, tailTexture, desrect.x, desrect.y, desrect.w, desrect.h, 180);
		}
		//check the snake go right
		else if (tail[(lastTail + i) % 10000].x == 23 && checkDirection[(lastTail + i) % 10000] == 2 &&
			tail[(lastTail + i + 1) % 10000].x == 0) {
...
void Snake::turnUp() {
	if (pause == false) {
		...
		if (velocity.Y != 1 && checkDuplicateDirectionUp != 1) {
			...
			velocity.Y = -1;
			headAngle = -90;
			checkDirection[firstTail % 10000] = 1;
			checkCorner[firstTail % 10000] = 1;
...
void Snake::turnRight() {
	if (pause == false) {
		...
		if (velocity.X != -1 && checkDuplicateDirectionRight != 1) {
			...
			velocity.Y = 0;
			headAngle = 0;
			checkDirection[firstTail % 10000] = 2;
			checkCorner[firstTail % 10000] = 3;
			//check duplicate direction
